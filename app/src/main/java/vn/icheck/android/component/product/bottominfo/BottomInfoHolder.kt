...
class BottomInfoHolder(parent: ViewGroup,val listener: ProductDetailListener) : BaseHolder(LayoutInflater.from(parent.context).inflate(R.layout.item_product_bottom_info, parent, false)) {
    ...
    fun bind(bottomModel: BottomModel) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            itemView.tvPhone.text = Html.fromHtml(ViewHelper.setPrimaryHtmlString(itemView.context.getString(R.string.tong_dai_icheck, productContact.find { it.key == "product-contact.phone" }?.value ?: "0902195488"),itemView.context), Html.FROM_HTML_MODE_COMPACT)
            itemView.tvEmail.text = Html.fromHtml(ViewHelper.setPrimaryHtmlString(itemView.context.getString(R.string.email_icheck, productContact.find { it.key == "product-contact.mail" }?.value ?: "cskh@icheck.vn"),itemView.context), Html.FROM_HTML_MODE_COMPACT)
            itemView.tvHotline.text = Html.fromHtml(ViewHelper.setPrimaryHtmlString(itemView.context.getString(R.string.hotline_dang_ky, productContact.find { it.key == "product-contact.hot-line" }?.value ?: "0902195488"),itemView.context), Html.FROM_HTML_MODE_COMPACT)
        } else {
            itemView.tvPhone.text = Html.fromHtml(ViewHelper.setPrimaryHtmlString(itemView.context.getString(R.string.tong_dai_icheck, productContact.find { it.key == "product-contact.phone" }?.value ?: "0902195488"),itemView.context))
            itemView.tvEmail.text = Html.fromHtml(ViewHelper.setPrimaryHtmlString(itemView.context.getString(R.string.email_icheck, productContact.find { it.key == "product-contact.mail" }?.value ?: "cskh@icheck.vn"),itemView.context))
            itemView.tvHotline.text = Html.fromHtml(ViewHelper.setPrimaryHtmlString(itemView.context.getString(R.string.hotline_dang_ky, productContact.find { it.key == "product-contact.hot-line" }?.value ?: "0902195488"),itemView.context))
        }
