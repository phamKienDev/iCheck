...
class SlideBannerAdapter(val listData: MutableList<ICAds>, private val topBannerListener: IBannerListener) : PagerAdapter() {
    ...
    override fun instantiateItem(container: ViewGroup, position: Int): Any {
        val itemView = ViewHelper.createBannerHolder(container.context)
        ...
        itemView.setOnClickListener {
            if (banner.type == Constant.BANNER_SURVEY) {
                ...
                ICheckApplication.currentActivity()?.let { activity ->
                    AdsUtils.bannerClicked(activity, listData[position])
                }
            }
        }
