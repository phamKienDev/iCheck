...
class LayoutInputOtp : LinearLayout {
    ...
    fun setReceiveOtpType(type: String) {
        ...
        when (receiveOtpType) {
            CALL_PHONE_TYPE -> {
                sendToPhoneMessage = context.getString(R.string.login_ma_xac_nhan_otp_da_duoc_thong_dai_goi_toi_so_dien_thoai, phoneNumber)
                receiverMessageType = context.getString(R.string.doi_phuong_thuc_nhan_sms)
            }
            SEND_SMS_TYPE -> {
                sendToPhoneMessage = context.getString(R.string.login_ma_xac_nhan_otp_da_duoc_gui_toi_so_dien_thoai, phoneNumber)
