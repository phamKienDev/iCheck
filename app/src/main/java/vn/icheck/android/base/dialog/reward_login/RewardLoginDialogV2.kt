...
class RewardLoginDialogV2(val listener: RewardLoginCallback) : DialogFragment() {
    ...
    companion object {
        ...
        fun show(fragmentManager: FragmentManager, callback: RewardLoginCallback) {
            if (fragmentManager.findFragmentByTag(RewardLoginDialogV2::class.java.simpleName)?.isAdded == true) {
                ...
                if (ICheckApplication.currentActivity() !is CheckThemeActivity) {
                    if (currentFragmentManager != fragmentManager.toString()) {
                        RewardLoginDialogV2(callback).apply {
                            show(fragmentManager, RewardLoginDialogV2::class.java.simpleName)
