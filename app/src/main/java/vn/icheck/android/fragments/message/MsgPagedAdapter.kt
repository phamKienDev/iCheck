...
import vn.icheck.android.util.UriUtils
import vn.icheck.android.util.kotlin.WidgetUtils

class MsgPagedAdapter : PagedListAdapter<MsgModel, RecyclerView.ViewHolder>(DIFF_CALLBACK) {

    companion object {
        private val DIFF_CALLBACK = object : DiffUtil.ItemCallback<MsgModel>() {
            override fun areItemsTheSame(oldItem: MsgModel, newItem: MsgModel): Boolean {
                return oldItem.getFirebaseId() == newItem.getFirebaseId()
            }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder.itemViewType) {
            MsgType.TYPE_USER_2_USER -> (holder as UserMsgHolder).bind(getItem(position))
            MsgType.TYPE_GROUP_MSG -> (holder as GroupMsgHolder).bind(getItem(position))
        }
    }
