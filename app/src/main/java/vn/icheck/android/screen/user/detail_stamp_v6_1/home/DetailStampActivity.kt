...
class DetailStampActivity : BaseActivity<DetailStampPresenter>(), IDetailStampView, CampaignLoyaltyHelper.ILoginListener, IClickListener, CampaignLoyaltyHelper.IRemoveHolderInputLoyaltyListener {
    ...
    private val requestCheckCode = 95
    private val requestGiftLoyalty = 99
...
//        initUpdateLocation()
    ...
    private fun getShopVariant(seller_id: Long, barcode: String?) {
        mFusedLocationClient?.lastLocation?.addOnSuccessListener {
            if (it != null) {
                ...
                presenter.getShopVariant(lat, lon, seller_id, barcode)
            }
    ...
    private fun initBanner() {
        bannerAdapter = BannerAdapter(this, this)
    ...
    private fun countDownToNext() {
        ...
        disposable = Observable.timer(3, TimeUnit.SECONDS)
                ...
                .subscribe {
                    viewPagerImgProduct?.let { viewPager ->
    ...
    private fun listener() {
        ...
        tvMoreHistoryGuarantee.setOnClickListener {
            ...
            if (tvSerialFake.text.toString() != null && tvSerialFake.text.toString().isNotEmpty()) {
                intent.putExtra(Constant.DATA_1, tvSerialFake.text.toString())
            } else if (tvSerialVerified.text.toString() != null && tvSerialVerified.text.toString().isNotEmpty()) {
                intent.putExtra(Constant.DATA_1, tvSerialVerified.text.toString())
            } else if (tvSerialVerifiedChongGia.text.toString() != null && tvSerialVerifiedChongGia.text.toString().isNotEmpty()) {
...
//            }
    ...
    override fun onGetDetailStampSuccess(obj: ICDetailStampV6_1) {
        if (!code.isNullOrEmpty()) {
            CampaignLoyaltyHelper.getCampaign(this@DetailStampActivity, code!!, this@DetailStampActivity, this@DetailStampActivity)
        }
        //Check Error Client
        if (obj == null) {
...
//                            val mNumber = textNumberSerial + it.number
                            ...
                            tvCountUserScanFake.text = if (!it.people_count.toString().isNullOrEmpty()) {
                    ...
                    1 -> {
                        ...
                        obj.data?.count?.let {
                            ...
                            tvCountUserScanVerified.text = if (!it.people_count.toString().isNullOrEmpty()) {
                                it.people_count.toString()
                            } else {
                                getString(R.string.dang_cap_nhat)
                            }
                    ...
                    2 -> {
                        ...
                        obj.data?.count?.let {
                            ...
                            tvCountUserScanVerifiedBaoHanh.text = if (!it.people_count.toString().isNullOrEmpty()) {
                                it.people_count.toString()
                            } else {
                                getString(R.string.dang_cap_nhat)
                            }
...
//      setdata slide image product
            if (obj.data?.product?.images != null) {
                ...
                if (!param.isNullOrEmpty()) {
                    ...
                    bannerAdapter?.setListImageData(obj.data?.product?.images!!, obj.data?.product?.video, videoImage)
                } else {
                    bannerAdapter?.setListImageData(obj.data?.product?.images!!, null, null)
                }
...
//                getGiftLoyalty()
            ...
            CampaignLoyaltyHelper.REQUEST_GET_GIFT -> {
                CampaignLoyaltyHelper.getCampaign(this@DetailStampActivity, code
    ...
    override fun onClick(obj: Any) {
        ...
        if (obj is ICKLoyalty) {
            loyalty_description.text = obj.name
            WidgetUtils.loadImageUrl(bannerLoyalty, obj.image?.original)
            btnCheckCode.setOnClickListener {
                btnCheckCode.isEnabled = false
                CampaignLoyaltyHelper.checkCodeLoyalty(this@DetailStampActivity, obj, edittextCode.text?.trim().toString(), code
                        ?: "", this@DetailStampActivity, this@DetailStampActivity)
                Handler().postDelayed({
            ...
            bannerLoyalty.setOnClickListener {
                startActivity(Intent(this@DetailStampActivity, UrlGiftDetailActivity::class.java).apply {
                    ...
                    putExtra(ConstantsLoyalty.DATA_2, code)
