...
class OtherNotificationComponent(parent: ViewGroup) : BaseViewHolder<MutableList<ICNotification>>(createView(parent.context)) {
    private val friendRequestAdapter = OtherNotificationAdapter()
        ...
        (itemView as ViewGroup).apply {
            ...
            (getChildAt(0) as ViewGroup).apply {
                ...
                (getChildAt(0) as AppCompatTextView).apply {
                    text = context.getString(R.string.thong_bao_khac)
                }

                // Text view more
                getChildAt(1).setOnClickListener {
                    ICheckApplication.currentActivity()?.let { activity ->
                        ActivityUtils.startActivity<ListOtherNotificationActivity>(activity)
                    }
                }
            }
