package vn.icheck.android.screen.user.detail_stamp_thinh_long.home.adapter
...
import vn.icheck.android.screen.user.detail_stamp_thinh_long.home.view_holder.*

class DetailStampThinhLongAdapter(private val headerImagelistener: SlideHeaderStampHoaPhatListener) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ICViewType.IMAGE_HEADER_STAMP -> {
                ImageHeaderStampThinhLongHolder(parent, headerImagelistener)
            }
            ICViewType.INFOR_HEADER_STAMP -> {
                InforHeaderStampThinhLongHolder(parent, headerImagelistener)
            }
...
//            }
            ICViewType.INFORMATION_PRODUCT_STAMP -> {
                InformationProductStampThinhLongHolder(parent, headerImagelistener)
            }
//            ICViewType.STEP_BUILD_PRODUCT_STAMP -> {
//                StepBuildProductStampHolder(parent, sharedPool)
//            }
            ICViewType.CERTIFICATE_STAMP -> {
                CeritificateStampHolder(parent, headerImagelistener)
            }
            ICViewType.DNSH_STAMP -> {
                DnshStampThinhLongHolder(parent, headerImagelistener)
            }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder) {
            is ImageHeaderStampThinhLongHolder -> {
                holder.bind(listData[position].data as ICBarcodeProductV1)
            }
            is InforHeaderStampThinhLongHolder -> {
                holder.bind(listData[position].data as ICBarcodeProductV1)
            }
...
//            }
            is InformationProductStampThinhLongHolder -> {
            ...
            is ProductRelatedStampThinhLongHolder -> {
