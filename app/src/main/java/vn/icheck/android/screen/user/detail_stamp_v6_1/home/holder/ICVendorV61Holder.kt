...
class ICVendorV61Holder(parent: ViewGroup, val binding: ItemVendorV61Binding = ItemVendorV61Binding.inflate(LayoutInflater.from(parent.context), parent, false)) : BaseViewHolder<ICWidgetData>(binding.root) {
    ...
    override fun bind(obj: ICWidgetData) {
        binding.layoutTop.setBackgroundColor(Constant.getPrimaryColor(itemView.context))
        binding.tvPhone.setTextColor(Constant.getPrimaryColor(itemView.context))
        binding.tvPhone.setHintTextColor(Constant.getPrimaryColor(itemView.context))
        binding.tvAddress.setTextColor(Constant.getPrimaryColor(itemView.context))
        binding.tvAddress.setHintTextColor(Constant.getPrimaryColor(itemView.context))
