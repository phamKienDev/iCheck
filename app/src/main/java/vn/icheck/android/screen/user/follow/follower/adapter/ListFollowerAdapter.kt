package vn.icheck.android.screen.user.follow.follower.adapter
...
class ListFollowerAdapter(val listener: IRecyclerViewAdapterListener<ICUserFollower>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    private val listData = mutableListOf<ICUserFollower>()
    ...
    fun setData(list: MutableList<ICUserFollower>) {
    ...
    fun addData(list: MutableList<ICUserFollower>) {
        onLoadSuccess(list.size)
            ...
            is LongMessageHolder -> {
                errorMessage?.let {
                    if (it.isEmpty()) {
                        ...
                        holder.bind(iconMessage, message, -1)
                    } else {
    ...
    class ViewHolder(view: View) : BaseViewHolder<ICUserFollower>(view) {
        override fun bind(obj: ICUserFollower) {
            WidgetUtils.loadImageUrl(itemView.imgAvatar, ImageHelper.getImageUrl(obj.account.avatar, obj.account.avatar_thumbnails?.small, ImageHelper.smallSize),
                    R.drawable.ic_square_avatar_default,
                    R.drawable.ic_square_avatar_default)

            itemView.tvName.text = obj.account.name
        }
    }
}