...
class FriendWallHolder(val binding: FriendInWallHolderBinding) : RecyclerView.ViewHolder(binding.root) {
    ...
    fun bind(model: IckUserFriendModel) {
        listener(model)
        if (model.type == 1) {
            itemView.tvCountFriend.rText(R.string.ban_be_d, model.listFriend.data?.count)
        } else {
            itemView.tvCountFriend.rText(R.string.ban_be_chung_d, model.listFriend.data?.count)
        }
