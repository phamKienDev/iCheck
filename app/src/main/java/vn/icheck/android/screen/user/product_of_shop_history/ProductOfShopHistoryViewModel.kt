...
class ProductOfShopHistoryViewModel : ViewModel() {
    ...
    val dataIntent = MutableLiveData<ICSuggestStoreHistory>()
    val listData = MutableLiveData<MutableList<ICProductOfShopHistory>>()
    ...
    fun getDataIntent(intent: Intent?) {
        val obj = try {
            intent?.getSerializableExtra(Constant.DATA_1) as ICSuggestStoreHistory
        } catch (e: Exception) {
            null
        }

        if (obj != null) {
            dataIntent.postValue(obj!!)
            idShop = obj.id!!
            getProductOfShopHistory(obj.id!!)
        }
    }

    fun getProductOfShopHistory(idShop: Long, isLoadMore: Boolean = false) {
        viewModelScope.launch {
            ...
            interactor.getProductOfShopHistory(offset, idShop, object : ICNewApiListener<ICResponse<ICListResponse<ICProductOfShopHistory>>> {
                override fun onSuccess(obj: ICResponse<ICListResponse<ICProductOfShopHistory>>) {
