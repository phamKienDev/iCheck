package vn.icheck.android.screen.user.detail_stamp_thinh_long.home.adapter.related_product
...
class RelatedProductStampThinhLongAdapter(val list: MutableList<ICRelatedProductV1.RelatedProductRow>, private val headerImagelistener: SlideHeaderStampHoaPhatListener) : RecyclerView.Adapter<RelatedProductStampThinhLongAdapter.RelatedProductStampHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RelatedProductStampHolder {
        return RelatedProductStampHolder(LayoutInflater.from(parent.context).inflate(R.layout.item_product_related_stamp_thinh_long, parent, false))
    }
    ...
    class RelatedProductStampHolder(view: View) : RecyclerView.ViewHolder(view) {
        fun bind(child: ICRelatedProductV1.RelatedProductRow) {
            ...
            if (child.price > 0) {
                itemView.product_price.rText(R.string.x_d, child.price)
            } else {
