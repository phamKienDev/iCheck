    ...
    fun show() {
        dialog.tvNameUnfollow.apply {
            text = if (!item.lastName.isNullOrEmpty() && !item.firstName.isNullOrEmpty()) {
                context.rText(R.string.bo_theo_doi_s_s, item.lastName, item.firstName)
            } else if (!item.firstName.isNullOrEmpty()) {
                context.rText(R.string.bo_theo_doi_s, item.firstName)
            } else if (!item.lastName.isNullOrEmpty()) {
                context.rText(R.string.bo_theo_doi_s, item.lastName)
            } else {
                dialog.context.getString(R.string.dang_cap_nhat)
            }
        }

        dialog.tvUnfriend.rText(R.string.huy_ket_ban_s, item.getUserName())
