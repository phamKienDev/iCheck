...
abstract class FilterUserDialog(val activity: FragmentActivity, val city: MutableList<ICProvince>?, val gender: MutableList<String>?) : BaseBottomSheetDialog(activity, R.layout.dialog_filter_user_search, true) {
    ...
    private fun setLocation(list: MutableList<ICProvince>?) {
        ...
        if (list.isNullOrEmpty()) {
            ...
            dialog.tv_location.setTextColor(ColorManager.getSecondTextColor(ICheckApplication.getInstance()))
        } else {
            if (list[0].id != -1L) {
                ...
                dialog.tv_location.setTextColor(ColorManager.getPrimaryColor(dialog.context))
    ...
    private fun setGender(gender: MutableList<String>?) {
        ...
        if (selectedGender.isNullOrEmpty() || selectedGender.size == 3) {
            ...
            dialog.tv_gender.setTextColor(ColorManager.getSecondTextColor(ICheckApplication.getInstance()))
        } else {
            ...
            dialog.tv_gender.setTextColor(ColorManager.getPrimaryColor(dialog.context))
        }
    }
