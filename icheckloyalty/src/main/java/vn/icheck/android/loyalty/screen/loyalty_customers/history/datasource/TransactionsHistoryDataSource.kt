...
class TransactionsHistoryDataSource(private val icNetworkAPI: ICNetworkAPI, val userId:Long, val count: MutableLiveData<Int>) : PagingSource<Int, TransactionHistoryResponse>() {
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, TransactionHistoryResponse> {
        return try {
            // Start refresh at page 1 if undefined.
            val nextPageNumber = params.key ?: 0
            val response = icNetworkAPI.getLoyaltyTransactionHistory(userId, nextPageNumber, 10)
            if (nextPageNumber == 0) {
