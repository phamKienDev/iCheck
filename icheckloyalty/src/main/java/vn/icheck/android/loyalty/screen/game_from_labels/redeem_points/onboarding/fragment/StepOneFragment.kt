...
class StepOneFragment(val listener: IOnboardingListener) : BaseFragmentGame() {
    ...
    override fun onInitView() {
        if (SharedLoyaltyHelper(requireContext()).getBoolean(ConstantsLoyalty.HAS_CHANGE_CODE_REDEEM_POINTS)) {
            ...
            tvTitle.setText(R.string.nhap_ma_may_man_tich_diem_day_kho)
            tvMessage.setText(R.string.ban_oi_nhap_ma_in_tren_san_pham_de_nhan_diem_tich_luy_doi_qua_nhe)
        } else {
            imgDefault.setImageResource(R.drawable.ic_onboarding_scan)
            tvTitle.setText(R.string.quet_qr_code_tich_diem_day_kho)
            tvMessage.setText(R.string.ban_oi_quet_qr_code_tren_san_pham_de_nhan_diem_tich_luy_doi_qua_nhe)
        }
