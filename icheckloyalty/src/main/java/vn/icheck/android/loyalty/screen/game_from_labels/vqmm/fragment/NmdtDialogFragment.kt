                ...
                luckyGameViewModel.receiveGame(edt_nmdt.text.toString().trim()).observe(viewLifecycleOwner, Observer { response ->
                    try {
                        if (response?.data?.play != null) {
                            ...
                            tv_error.text = response?.data?.message ?: rText(R.string.ma_s_da_duoc_su_dung, edt_nmdt.text)
                        }
                    } catch (e: Exception) {
                        tv_error.visibility = View.VISIBLE
                        tv_error.rText(R.string.ma_s_da_duoc_su_dung, edt_nmdt.text)
                        root.startAnimation(AnimationUtils.loadAnimation(requireContext(), R.anim.error_shake))
                    }
                })
            }else{
                tv_error.visibility = View.VISIBLE
                tv_error.rText(R.string.ban_chua_nhap_ma_du_thuong)
                root.startAnimation(AnimationUtils.loadAnimation(requireContext(), R.anim.error_shake))
            }
        }
    }
