                ...
                if (!isLoadMore) {
                    if (obj.data?.rows.isNullOrEmpty()) {
                        if (SharedLoyaltyHelper(ApplicationHelper.getApplicationByReflect()).getBoolean(ConstantsLoyalty.HAS_CHANGE_CODE_VQMM)){
                            setErrorEmpty(R.drawable.ic_default_loyalty, rText(R.string.ban_chua_co_qua_nao), rText(R.string.thu_nhap_ngay_biet_dau_van_may_toi), rText(R.string.nhap_ma_ngay), R.drawable.bg_corner_53_no_solid_stroke_1_blue, R.color.colorSecondary)
                        }else{
                            setErrorEmpty(R.drawable.ic_default_loyalty, rText(R.string.ban_chua_co_qua_nao), rText(R.string.thu_quet_ngay_biet_dau_van_may_toi), rText(R.string.quet_ma_ngay), R.drawable.bg_corner_53_no_solid_stroke_1_blue, R.color.colorSecondary)
                        }
                    } else {
